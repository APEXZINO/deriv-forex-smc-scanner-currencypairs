"""
Deriv Forex SMC+ICT Scanner — OB + FVG Confluence Model
Pairs: GBPUSD, EURUSD, XAUUSD, USDJPY, GBPJPY
Runs 24/7 on Railway.app

This is a direct replica of the synthetic-indices SMC OB+FVG scanner,
re-pointed at Deriv's real forex feed (frx-prefixed symbols) instead
of synthetic indices. All entry logic, scoring, and trade math are
unchanged from the original.

Entry Logic (all must align):
  H4  — Trend bias (EMA 21/50) + ADX strength
  H1  — Unmitigated fresh Order Block (last 20 bars)
  M15 — Fair Value Gap overlapping inside the H1 OB zone
...
IMPORTANT — SYMBOL VERIFICATION:
Deriv's forex symbols use an "frx" prefix (e.g. frxGBPUSD). Gold is
typically listed under Deriv's commodities/metals group rather than
the standard forex group, and its exact symbol string can vary by
account/region. On startup this script calls Deriv's `active_symbols`
endpoint and verifies every symbol in FOREX_SYMBOLS actually exists
before scanning. If a symbol is missing (most likely XAUUSD), it will
log the closest matches found so you can correct the mapping below.
"""
...
# Deriv forex symbol mapping: { display_name: deriv_symbol }
# frx-prefixed symbols are Deriv's standard forex naming convention.
# XAUUSD is included with its most common Deriv symbol string; this is
# verified against active_symbols on startup (see verify_symbols()).
SYMBOL_MAP = {
    "GBPUSD": "frxGBPUSD",
    "EURUSD": "frxEURUSD",
    "XAUUSD": "frxXAUUSD",
...
# Timeframes (unchanged from synthetics version)
...
# FVG settings
# NOTE: forex pairs have much smaller absolute price ranges than
# synthetic indices, but FVG_MIN_PCT is a *percentage* of price so it
# scales naturally — no change needed from the synthetics version.
FVG_MIN_PCT  = 0.02
...
# JPY pairs trade with 2-3 decimals, majors with 4-5, gold with 2.
DECIMALS = {
    "GBPUSD": 5,
    "EURUSD": 5,
    "XAUUSD": 2,
...
async def verify_symbols() -> dict:
    ...
    symbol that isn't found, most likely XAUUSD if Deriv's gold symbol
    differs from "frxXAUUSD" for this account/region.
    """
    valid_map = {}
    try:
        async with websockets.connect(WS_URI, ping_timeout=15, open_timeout=20) as ws:
            ...
            for display_name, deriv_symbol in SYMBOL_MAP.items():
                if deriv_symbol in all_symbols:
                    ...
                    # Look for close matches to help diagnose naming issues
                    candidates = [
...
def build_alert(
    ...
    return (
        ...
        f"<code>{div}</code>\n"
        f"<i>H4 trend + H1 OB + M15 FVG + M5 entry all confirmed.</i>"
    )


# =============================================================================
#  WEBSOCKET — candle fetcher (unchanged logic, works identically for forex)
# =============================================================================
...
# =============================================================================
#  INDICATORS (unchanged from synthetics version)
# =============================================================================
...
async def scan_symbol(display_symbol: str, deriv_symbol: str) -> None:
    ...
    # Layer 2: H1 Order Block
    ob = find_ob(h1, bias)
    if not ob:
        log.info("%s | No valid H1 OB found.", display_symbol)
        return
    log.info("%s | H1 OB: %.6f – %.6f  (age: %d bars)",
             display_symbol, ob["lo"], ob["hi"], ob["age"])

    # Layer 3: M15 FVG inside OB
    fvg = find_fvg_in_ob(m15, bias, ob)
    if not fvg:
        log.info("%s | No M15 FVG overlapping H1 OB zone.", display_symbol)
        return
    log.info("%s | M15 FVG: %.6f – %.6f", display_symbol, fvg["lo"], fvg["hi"])

    # Confluence zone
    zone_lo = max(ob["lo"], fvg["lo"])
    zone_hi = min(ob["hi"], fvg["hi"])
    if zone_lo >= zone_hi:
        zone_lo = max(fvg["lo"], ob["lo"])
        zone_hi = min(fvg["hi"], ob["hi"])
        if zone_lo >= zone_hi:
            zone_lo, zone_hi = fvg["lo"], fvg["hi"]
...
async def main() -> None:
    ...
    log.info("=" * 60)
    log.info("Forex SMC+ICT OB+FVG Scanner  |  Started %s", start_time)
    log.info("=" * 60)

    # Verify symbols before starting the scan loop
    ...
    send_telegram(
        f"🚀 <b>Forex SMC+ICT OB+FVG Scanner LIVE</b>\n"
        f"<b>Pairs:</b> {', '.join(active_map.keys())}\n"
        f"<b>Model:</b> H4 Trend → H1 OB → M15 FVG → M5 Entry\n"
        f"<b>Started:</b> {start_time}\n"
        f"<i>Scanning every 5 min. Cooldown: 4 h per level.</i>"
    )
